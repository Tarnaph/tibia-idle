...
let cachedOtbMap: Map<number, { clientId: number; group: number }> | null = null;
let cachedBufferRef: Buffer | null = null;

export function parseOtbMap(buffer: Buffer): Map<number, { clientId: number; group: number }> {
  if (cachedBufferRef === buffer && cachedOtbMap) return cachedOtbMap;
  const map = new Map<number, { clientId: number; group: number }>();
  ...
  for (const child of root.children) {
    ...
    if (serverId !== undefined && clientId !== undefined) {
      map.set(serverId, { clientId, group: child.type });
    }
  }
...
export function findOtbClientId(buffer: Buffer, requestedServerId: number): { clientId: number; group: number } {
  const map = parseOtbMap(buffer);
  const found = map.get(requestedServerId);
...
export function tryFindOtbClientId(
  ...
  return parseOtbMap(buffer).get(requestedServerId);
}

...
import type { ItemEconomyCatalog, ItemSellOffer, MonsterCatalog, RealMapItemSellOffer } from '../../content-schema/src/index.ts';
...
export async function importEconomy(options: ImportOptions = {}): Promise<ItemEconomyCatalog> {
  ...
  const npcRoot = resolve(serverRoot, 'data', 'npc');
  const offers = new Map<number, RealMapItemSellOffer[]>();
  const push = (itemId: number, offer: RealMapItemSellOffer) => offers.set(itemId, [...(offers.get(itemId) ?? []), offer]);
  const parser = new XMLParser({ ignoreAttributes: false, attributeNamePrefix: '', parseAttributeValue: false, trimValues: true });

  if (existsSync(npcRoot)) {
    ...
    const taskLuaPath = resolve(npcRoot, 'scripts', 'task.lua');
    if (existsSync(taskLuaPath)) {
      const taskSource = await readFile(taskLuaPath, 'utf8');
  ...
  const catalog: ItemEconomyCatalog = { importedAtBuildTime: true, canonicalRule: 'realmap11-first-then-web-highest-proven-sell-price', items };

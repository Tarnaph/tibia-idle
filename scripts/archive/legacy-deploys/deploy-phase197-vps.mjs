...
conn.on('ready', () => {
  console.log('[SSH] Conectado à VPS 187.7.16.210. Iniciando deploy da Fase 197 (FIX.md: Highscores, Treino Tab, Hotbar Potion, Set Outfit, Topbar/Dock buttons, Ctrl+Arrows Body Turn)...');
  const cmd = `
    set -e
    cd /root/tibia-idle
    ...
    BACKUP_FILE="/root/db_backups/deploy_backups/dev_pre_phase197_deploy_$(date +%Y%m%d_%H%M%S).db"
    ...
    echo "=== 4.1 PRUNE TEST CHARACTERS & ACCOUNTS FROM DB ==="
    node scripts/prune-test-characters.cjs
    ...
    NODE_OPTIONS="--max-old-space-size=4096" npx vinext build
    ...
    echo ""
    echo "=== DEPLOY DA FASE 197 CONCLUÍDO COM SUCESSO NA VPS (Commit $LATEST_COMMIT) ==="
  `;

...
conn.on('ready', () => {
  console.log('[SSH] Conectado à VPS 187.7.16.210. Iniciando deploy da Fase 207 (Character Deduplication & Robust Hunt State Sync)...');
  const cmd = `
    set -e
    cd /root/tibia-idle
    ...
    BACKUP_FILE="/root/db_backups/deploy_backups/dev_pre_phase207_deploy_$(date +%Y%m%d_%H%M%S).db"
    ...
    echo ""
    echo "=== DEPLOY DA FASE 207 CONCLUÍDO COM SUCESSO NA VPS (Commit $LATEST_COMMIT) ==="
  `;

...
conn.on('ready', () => {
  console.log('[SSH] Conectado à VPS 187.7.16.210. Iniciando deploy da Fase 192 (Ranked PvP Arena, Skulls & Matchmaking System)...');
  const cmd = `
    set -e
    cd /root/tibia-idle
    ...
    BACKUP_FILE="/root/db_backups/deploy_backups/dev_pre_phase192_deploy_$(date +%Y%m%d_%H%M%S).db"
    ...
    echo "=== 5. GENERATE SKULL SPRITES ON VPS ==="
    ...
    echo ""
    echo "=== DEPLOY DA FASE 192 CONCLUÍDO COM SUCESSO NA VPS (Commit $LATEST_COMMIT) ==="
  `;

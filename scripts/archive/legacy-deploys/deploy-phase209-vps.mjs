...
conn.on('ready', () => {
  console.log('[SSH] Conectado à VPS 187.7.16.210. Iniciando deploy da Fase 209 (Audio & Sound Effects - SFX System)...');
  const cmd = `
    set -e
    cd /root/tibia-idle
    ...
    BACKUP_FILE="/root/db_backups/deploy_backups/dev_pre_phase209_deploy_$(date +%Y%m%d_%H%M%S).db"
    ...
    echo "=== 6. VERIFY CANONICAL SFX FILES ==="
    ls -lh public/assets/sfx/
    ...
    echo ""
    echo "=== DEPLOY DA FASE 209 CONCLUÍDO COM SUCESSO NA VPS (Commit $LATEST_COMMIT) ==="
  `;

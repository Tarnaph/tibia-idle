...
conn.on('ready', () => {
  console.log('[SSH] Conectado à VPS 187.7.16.210. Iniciando migração e deploy para retirar Premium de todos os players...');
  const cmd = `
    set -e
    cd /root/tibia-idle
    ...
    BACKUP_FILE="/root/db_backups/deploy_backups/dev_pre_remove_premium_$(date +%Y%m%d_%H%M%S).db"
    cp -a prisma/dev.db "$BACKUP_FILE"
    echo "Backup created at: $BACKUP_FILE"
    
    echo "=== 2. GIT FETCH & RESET TO ORIGIN/MAIN ==="
    git fetch origin main
    git reset --hard origin/main
    LATEST_COMMIT=$(git rev-parse --short HEAD)
    echo "HEAD now at: $LATEST_COMMIT"
    
    echo "=== 3. PRISMA SCHEMA SYNC & GENERATE ==="
    npx prisma db push --accept-data-loss
    npx prisma generate
    
    echo "=== 4. EXECUTE REMOVE PREMIUM SCRIPT ON VPS DATABASE ==="
    node scripts/remove-premium-players.mjs
    ...
    echo ""
    echo "=== MIGRAÇÃO E DEPLOY CONCLUÍDOS COM SUCESSO NA VPS (Commit $LATEST_COMMIT) ==="
  `;
  ...
  conn.exec(cmd, (err, stream) => {
    ...
    stream.on('close', (code) => {
      console.log(`\n[SSH] Script finalizado com código ${code}`);
      conn.end();
      process.exit(code || 0);
    });
  });

...
conn.on('ready', () => {
  console.log('[SSH] Conectado à VPS 187.7.16.210. Iniciando deploy da Fase 210 (Quick Sell Persistent Item Selection)...');
  const cmd = `
    set -e
    cd /root/tibia-idle
    ...
    BACKUP_FILE="/root/db_backups/deploy_backups/dev_pre_phase210_deploy_$(date +%Y%m%d_%H%M%S).db"
    ...
    echo ""
    echo "=== DEPLOY DA FASE 210 CONCLUÍDO COM SUCESSO NA VPS (Commit $LATEST_COMMIT) ==="
  `;

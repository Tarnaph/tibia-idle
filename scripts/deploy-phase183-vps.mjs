...
conn.on('ready', () => {
  console.log('[SSH] Conectado à VPS 187.7.16.210. Iniciando deploy da Fase 183 (Shop Fix: FIX.md + Exercise Weapons + Canonical Cyclopedia)...');
  const cmd = `
    set -e
    cd /root/tibia-idle
    ...
    BACKUP_FILE="/root/db_backups/deploy_backups/dev_pre_phase183_deploy_$(date +%Y%m%d_%H%M%S).db"
    ...
    echo "=== 3.1 MIRROR EXERCISE & CYCLOPEDIA SPRITES TO TIBIA1098 & ASSETS ==="
    mkdir -p /root/tibia-idle/public/generated/tibia1098/items
    mkdir -p /root/tibia-idle/public/assets/items
    cp -a /root/tibia-idle/public/generated/cyclopedia/items/item-*.png /root/tibia-idle/public/generated/tibia1098/items/ 2>/dev/null || true
    cp -a /root/tibia-idle/public/generated/cyclopedia/items/item-*.png /root/tibia-idle/public/assets/items/ 2>/dev/null || true
    echo "Sprites mirrored successfully."
    ...
    sqlite3 prisma/dev.db "PRAGMA integrity_check;"
    echo ""
    echo "=== DEPLOY DA FASE 183 CONCLUÍDO COM SUCESSO NA VPS (Commit $LATEST_COMMIT) ==="
  `;

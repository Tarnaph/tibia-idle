...
conn.on('ready', () => {
  console.log('[SSH] Conectado à VPS 187.7.16.210. Iniciando deploy da Fase 184 (TopBar Redesign, Real-time Online Players, Exura Coins)...');
  const cmd = `
    set -e
    cd /root/tibia-idle
    ...
    BACKUP_FILE="/root/db_backups/deploy_backups/dev_pre_phase184_deploy_$(date +%Y%m%d_%H%M%S).db"
    ...
    sqlite3 prisma/dev.db "PRAGMA integrity_check;"
    echo ""
    echo "=== DEPLOY DA FASE 184 CONCLUÍDO COM SUCESSO NA VPS (Commit $LATEST_COMMIT) ==="
  `;

...
conn.on('ready', () => {
  console.log('[SSH] Conectado à VPS 187.7.16.210. Iniciando deploy da Fase 204 (Resolução Completa do PVP, Loading Loop, ELO 0, Caveira Canônica Tibia 11x11, Visibilidade Thais)...');
  const cmd = `
    set -e
    cd /root/tibia-idle
    ...
    BACKUP_FILE="/root/db_backups/deploy_backups/dev_pre_phase204_deploy_$(date +%Y%m%d_%H%M%S).db"
    ...
    echo "=== 4. PRISMA SCHEMA & CLIENT GENERATE ==="
    npx prisma db push --skip-generate
    npx prisma generate
    
    echo "=== 5. MIGRATE LEGACY PVP ELO DEFAULTS ==="
    sqlite3 prisma/dev.db "UPDATE characters SET pvpElo = 0, pvpTier = 'Iniciante' WHERE pvpElo = 1000;"
    sqlite3 prisma/dev.db "SELECT id, name, pvpElo, pvpTier, displaySkull FROM characters LIMIT 5;"
    ...
    echo "=== 7. BUILD PRODUCTION BUNDLE (npx vinext build) ==="
    npx vinext build
    ...
    echo ""
    echo "=== DEPLOY DA FASE 204 CONCLUÍDO COM SUCESSO NA VPS (Commit $LATEST_COMMIT) ==="
  `;

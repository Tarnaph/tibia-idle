...
conn.on('ready', () => {
  console.log('[SSH] Conectado à VPS 187.7.16.210. Iniciando deploy da Fase 216 (Combat FX Atlas & PixiJS v8 Texture Resolution)...');
  const cmd = `
    set -e
    cd /root/tibia-idle
    ...
    BACKUP_FILE="/root/db_backups/deploy_backups/dev_pre_phase216_deploy_$(date +%Y%m%d_%H%M%S).db"
    ...
    echo "=== 4. BUILD COMBAT ATLAS ON VPS ==="
    node scripts/build-combat-atlas.mjs || true
    
    echo "=== 5. PRISMA CLIENT GENERATE ==="
    npx prisma generate
    ...
    echo ""
    echo "=== DEPLOY DA FASE 216 CONCLUÍDO COM SUCESSO NA VPS (Commit $LATEST_COMMIT) ==="
  `;

    ...
    <aside className={`client-sidebar right-sidebar ${mobileOpen ? 'mobile-open' : ''}`}>
      ...
      <ClientPanel title="Backpack" meta={`${inventoryEquipment.length} slots`} className="backpack-module">
        ...
        <div className="inventory-grid equipment-inventory-grid" data-equipment-drop="inventory">
          {inventoryEquipment.map((item) => <button type="button" key={item.id} className="inventory-cell" onPointerDown={(event) => onPointerDragStart({ kind: 'inventory', itemId: item.id }, event)} onClick={() => onTransfer({ kind: 'inventory', itemId: item.id }, { kind: 'auto-slot' })} onMouseEnter={(e) => showGlobalItemTooltip({ item, itemId: item.id, name: item.name }, e)} onMouseMove={(e) => showGlobalItemTooltip({ item, itemId: item.id, name: item.name }, e)} onMouseLeave={() => hideGlobalItemTooltip()} aria-label={`Equipar ${item.name}`}><ItemSprite itemId={item.id} label={item.name} /></button>)}
      ...
      <ClientPanel title="Loot Pouch" meta={`${lootTotal} un.`} className="loot-module">
        ...
        {loot.length === 0 ? <div className="module-empty"><span>◇</span><small>Nenhum drop acumulado.</small></div> : <div className="inventory-grid loot-grid">{loot.map((stack) => {
          ...
          return <div onContextMenu={(event) => { event.preventDefault(); setMenu({ x: event.clientX, y: event.clientY, stack }); }} onMouseEnter={(e) => showGlobalItemTooltip({ itemId: stack.itemId, name: stack.name, amount: stack.amount, price: price ?? undefined, lockSell: preference?.lockSell, quickSell: preference?.quickSell }, e)} onMouseMove={(e) => showGlobalItemTooltip({ itemId: stack.itemId, name: stack.name, amount: stack.amount, price: price ?? undefined, lockSell: preference?.lockSell, quickSell: preference?.quickSell }, e)} onMouseLeave={() => hideGlobalItemTooltip()} className={`inventory-cell loot-cell ${price === null ? 'unsellable' : ''} ${preference?.lockSell ? 'locked' : ''} ${preference?.quickSell ? 'quick-sell' : ''}`} key={`${stack.itemId ?? 'name'}-${stack.name}`}><ItemSprite itemId={stack.itemId} label={stack.name} /><b>{stack.amount}</b></div>;
        })}</div>}
